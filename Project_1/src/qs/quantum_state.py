...
from samplers.metro_fermions import Metropolis_fermions as Metro_fermions
from samplers.metro_hastings import MetropolisHastings as MetroHastings
from samplers.metro_hast_fermions import MetropolisHastings_fermions as MetroHastings_fermions
...
class QS:
    ...
    def set_sampler(self, mcmc_alg, scale=0.5):
        ...
        if self.mcmc_alg == "m":
            ...
            if self._particle_type == "fermions":
                self.sampler = Metro_fermions(
                    vmc_instance,
                    hami,
                    self.rng,
                    self._scale,
            ...
            if self._particle_type == "fermions":
                self.sampler = MetroHastings_fermions(
                    vmc_instance,
                    hami,
                    self.rng,
                    self._scale,
        ...
        with tqdm(
            ...
            for iteration in range(max_iter):
                ...
                self.alpha = self.backend.array(
                    self._optimizer.step([self.alpha], [grads_alpha] , ite = iteration)
                )[0]

                # Update the progressbar to show the current alpha value
                pbar.set_description(
                    rf"[Training progress, alpha={float(self.alpha):.4f} , energy={float(self.backend.mean(local_energies)):.4f}]"
                )
                pbar.update(1)
                # Update the alpha in the Parameter instance
                old_alpha = self.alg.params.get("alpha")
                diff_alpha = self.backend.abs(self.alpha - old_alpha)
                if diff_alpha < tol:
