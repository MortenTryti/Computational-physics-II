...
class HarmonicOscillator(Hamiltonian):
    ...
    def CoulombInteraction(self, r):
        ...
        r_dist = self.backend.where(r_dist < config.radius, 10**-11, r_dist)     
        #breakpoint()
    ...
    def hard_shell_interaction(self, r):

        r_copy = r.copy()
        a, b = self.backend.triu_indices(r_copy[:,0].size,1)
        r_dist = self.la.norm(r_copy[a] - r_copy[b], axis=1)

        r_dist = self.backend.where(r_dist < config.radius, 10**-11,  10**11) 
    ...
    def non_int_energy(self, r):
        ...
        kinetic_energy =  - 0.5 * self.alg_int.laplacian(r)

        potential_energy  = 0.5 * self.backend.sum(self.beta**2 * r[:, 0]**2 + self.backend.sum(r[:, 1:]**2, axis=1))

        non_int_energy = kinetic_energy + potential_energy

        return non_int_energy
    
    

    def gradient_tot(self, r_up , r_down , D_inv_up , D_inv_down):

        grad_up = jnp.einsum("ijk , ji -> jk " , self.alg_int.grad_wf(r_up , self.n_up), D_inv_up) 
        grad_down = jnp.einsum("ijk , ji -> jk " ,self.alg_int.grad_wf(r_down , self.n_down), D_inv_down) 

        return grad_up , grad_down
    
    def laplacian_tot(self, r_up , r_down , D_inv_up , D_inv_down):

        lap_up = jnp.einsum(" ij , ji -> j" , self.alg_int.laplacian(r_up , self.n_up), D_inv_up)
        lap_down = jnp.einsum(" ij , ji -> j " , self.alg_int.laplacian(r_down , self.n_down), D_inv_down ) 


        grad_up , grad_down = self.gradient_tot(r_up , r_down , D_inv_up , D_inv_down)

        laplacian_tot = lap_up + lap_down - jnp.sum(grad_up**2 , axis=1) - jnp.sum(grad_down**2 , axis=1)

        return  laplacian_tot
    ...
    def local_energy(self, wf, r):
        ...
        elif self._int_type == "Hardshell":
            interaction_energy = self.hard_shell_interaction(r)
        else:
            pass
